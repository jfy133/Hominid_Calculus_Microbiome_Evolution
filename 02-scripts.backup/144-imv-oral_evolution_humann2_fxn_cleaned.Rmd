...
# This takes a very long time to load
gene_families.humann2.uniref <- fread("grep -v '|' /Users/velsko/projects1/microbiome_calculus/evolution/04-analysis/screening/humann2/tables/humann2_pathabundance_20190408_nospace.tsv")
...
# save(gene_families.humann2.uniref, file = "../04-analysis/screening/humann2/tables/gene_families.humann2.uniref.RData")
# 
...
read_assignments.humann2.uniref <- gene_families.humann2.uniref_filtered %>%
  dplyr::filter(str_detect(Pathways, "UNMAPPED|Total_reads")) %>%
  gather("individual_id","CPM", 2:ncol(.)) %>%
  spread(Pathways, CPM, 2:ncol(.)) %>%
...
H2KO.pca_df$Host_Genus <- factor(H2KO.pca_df$Host_Genus, 
                                 levels = c("Alouatta", "Gorilla", "Pan", "Homo", "Plaque"))
...
pca_H2KO_pc1pc2_outrem_all_nopq_pc1bi <- H2KO.pca_df %>%
  mutate(PC1 = PC1 / corr_lam[1],
         PC2 = PC2 / corr_lam[2]) %>%
  ggplot(.,
         aes(x = PC1, y = PC2, colour = Host_Genus)) +
  geom_point(size = 2, aes(colour = Host_Genus, shape = Host_Genus)) +
...
labs(x = paste("PC1 -", pca_expl_vars[1]),
  ...
  scale_colour_manual(values = genus_colours[c(1, 2, 3, 4, 4)]) +
...
# make Host Genus a factor for plotting
H2KO_nomod.pca_df$Host_Genus <- factor(H2KO_nomod.pca_df$Host_Genus, 
                                 levels = c("Alouatta", "Gorilla", "Pan", "Homo", "Plaque"))
