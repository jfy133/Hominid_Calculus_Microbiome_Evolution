...
```{r}
metadata_sample <- read_csv(paste0(proj_dir, "00-documentation.backup/01-calculus_microbiome_deep_evolution_samplescontrols_metadata_20190523.csv"))
...
metadata_library_screening %>%
  ...
  group_by(Group) %>%
  summarise_at(vars(Raw_Reads, Post_Clip_and_Merge, Percent_Endogenous_DNA), list(Mean = mean, SD = sd, Min = min, Max = max)) %>%
  gather(Metric, Value, 2:ncol(.)) %>% 
