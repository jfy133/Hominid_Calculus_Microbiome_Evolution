...
data_scr_samples <- data_scr_all %>%   
  select(individual,
         ...
         polyg_endogenous_dna_percent,
         cluster_factor, 
         median_fragment_length,
         polyg_median_fragment_length, 
         gc_content_percent, 
         polyg_gc_content_in_percent,
         non_human_reads) %>%
  mutate(
...
of the downstream analysis however, I will therefore only report the polyG 
...
TODO: Try loading new metadata file and add pasting of string columns
...
data_scr_indivs <- data_scr_samples %>% 
  ...
  summarise(total_raw_reads = sum(raw_reads), 
            ...
            mean_polyg_endogenous_dna_percent = mean(polyg_endogenous_dna_percent),
            mean_cluster_factor = mean(cluster_factor), 
            mean_median_fragment_length = mean(median_fragment_length),
            mean_polyg_median_fragment_length = mean(polyg_median_fragment_length), 
            mean_gc_content_percent = mean(gc_content_percent), 
            mean_polyg_gc_content_in_percent  = mean(polyg_gc_content_in_percent),
...
```{r}
data_scr_indivis_new <- data_scr_indivs %>% filter(study == "This_study", env != "ModernDayHumans_1")

data_scr_indivis_calculus <- data_scr_indivs %>% filter(study == "This_study", env != "ModernDayHumans_1")
```
